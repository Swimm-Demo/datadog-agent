...
---
In this document, we will explain the process of starting the security agent. The process involves initializing the agent, setting up necessary resources, and handling signals for stopping the agent.

The flow begins with the start function, which initializes the security agent by calling <SwmToken path="cmd/security-agent/subcommands/start/command.go" pos="209:5:5" line-data="	err := RunAgent(log, config, telemetry, statusComponent, settings, wmeta)">`RunAgent`</SwmToken>. <SwmToken path="cmd/security-agent/subcommands/start/command.go" pos="209:5:5" line-data="	err := RunAgent(log, config, telemetry, statusComponent, settings, wmeta)">`RunAgent`</SwmToken> sets up core dumps, checks for enabled components, and starts the API server. If there are issues like missing API keys, it exits gracefully. Otherwise, it sets up a signal handler to listen for stop signals and blocks until such a signal is received. When a stop signal is received, the <SwmToken path="cmd/security-agent/subcommands/start/command.go" pos="207:3:3" line-data="	defer StopAgent(log)">`StopAgent`</SwmToken> function is called to clean up resources and shut down the agent.
...
b57ade422e3b40cae17051c4aad1d49c7f2ca186f0e746ff9b22a2025dc14815(start):::mainFlowStyle --> 2fc71558cfbb58153b3e1a94a25379ae8ed975f4ea57606ae7e3a88e328bb93a(StopAgent)
end

subgraph cmdsecurityagentsubcommandsstartcommandgo["cmd/security-agent/subcommands/start/command.go"]
b57ade422e3b40cae17051c4aad1d49c7f2ca186f0e746ff9b22a2025dc14815(start):::mainFlowStyle --> 071b068f0057baf675100b3bb3eb7d8c27de65acc7a14ec2cc95d9ad3a4475b9(RunAgent):::mainFlowStyle
end
...
071b068f0057baf675100b3bb3eb7d8c27de65acc7a14ec2cc95d9ad3a4475b9(RunAgent):::mainFlowStyle --> 49gni(...)
...
<SwmSnippet path="/cmd/security-agent/subcommands/start/command.go" line="201">

---

## Starting the Security Agent

The <SwmToken path="cmd/security-agent/subcommands/start/command.go" pos="201:2:2" line-data="// start will start the security-agent.">`start`</SwmToken> function is responsible for initiating the security agent. It first calls <SwmToken path="cmd/security-agent/subcommands/start/command.go" pos="209:5:5" line-data="	err := RunAgent(log, config, telemetry, statusComponent, settings, wmeta)">`RunAgent`</SwmToken> to start the agent's main processes. If there are errors related to disabled components or missing API keys, it exits gracefully. Otherwise, it sets up a signal handler to listen for stop signals and blocks until such a signal is received.
...
## Stopping the Security Agent

The <SwmToken path="cmd/security-agent/subcommands/start/command.go" pos="321:2:2" line-data="// StopAgent stops the API server and clean up resources">`StopAgent`</SwmToken> function handles the cleanup and shutdown process of the security agent. It retrieves the health status of components using <SwmToken path="cmd/security-agent/subcommands/start/command.go" pos="324:3:3" line-data="	// GetReadyNonBlocking has a 100ms timeout to avoid blocking">`GetReadyNonBlocking`</SwmToken> and logs any unhealthy components. It then stops various services like <SwmToken path="cmd/security-agent/subcommands/start/command.go" pos="332:5:5" line-data="	// stop metaScheduler and statsd if they are instantiated">`metaScheduler`</SwmToken>, <SwmToken path="cmd/security-agent/subcommands/start/command.go" pos="332:9:9" line-data="	// stop metaScheduler and statsd if they are instantiated">`statsd`</SwmToken>, and <SwmToken path="cmd/security-agent/subcommands/start/command.go" pos="340:3:3" line-data="	if expvarServer != nil {">`expvarServer`</SwmToken> if they are running, ensuring a clean shutdown.
...
The <SwmToken path="pkg/status/health/global.go" pos="88:2:2" line-data="// GetReadyNonBlocking returns the health of all components registered for both readiness and liveness with a 500ms timeout">`GetReadyNonBlocking`</SwmToken> function returns the health status of all registered components with a <SwmToken path="pkg/status/health/global.go" pos="88:32:32" line-data="// GetReadyNonBlocking returns the health of all components registered for both readiness and liveness with a 500ms timeout">`500ms`</SwmToken> timeout. This is used by <SwmToken path="cmd/security-agent/subcommands/start/command.go" pos="207:3:3" line-data="	defer StopAgent(log)">`StopAgent`</SwmToken> to log the health status before shutting down the agent.
...
subgraph cmdsecurityagentsubcommandsstartcommandgo["cmd/security-agent/subcommands/start/command.go"]
61a4292f7434c5cd9432e402972d376039da1e16b0068c1806720662783b5f09(setupInternalProfiling):::mainFlowStyle --> g6nyi(...)
end
...
## <SwmToken path="cmd/security-agent/subcommands/start/command.go" pos="259:2:2" line-data="// RunAgent initialized resources and starts API server">`RunAgent`</SwmToken>

The <SwmToken path="cmd/security-agent/subcommands/start/command.go" pos="259:2:2" line-data="// RunAgent initialized resources and starts API server">`RunAgent`</SwmToken> function initializes necessary resources and starts the API server. It first sets up core dumps and checks if any <SwmToken path="cmd/security-agent/subcommands/start/command.go" pos="267:8:10" line-data="		log.Infof(&quot;All security-agent components are deactivated, exiting&quot;)">`security-agent`</SwmToken> components are enabled. If no components are enabled or no API key is configured, it logs the issue and exits. It then sets up an expvar server for telemetry and creates a new API server using the <SwmToken path="cmd/security-agent/api/server.go" pos="41:2:2" line-data="// NewServer creates a new Server instance">`NewServer`</SwmToken> function. Finally, it starts the API server and sets up internal profiling.
...
## <SwmToken path="cmd/security-agent/api/server.go" pos="41:2:2" line-data="// NewServer creates a new Server instance">`NewServer`</SwmToken>

The <SwmToken path="cmd/security-agent/api/server.go" pos="41:2:2" line-data="// NewServer creates a new Server instance">`NewServer`</SwmToken> function creates a new server instance. It first creates a new listener using the <SwmToken path="cmd/security-agent/api/server.go" pos="43:8:8" line-data="	listener, err := newListener()">`newListener`</SwmToken> function. If successful, it returns a new <SwmToken path="cmd/security-agent/api/server.go" pos="41:10:10" line-data="// NewServer creates a new Server instance">`Server`</SwmToken> instance with the listener and a new agent initialized with the provided components.
...
The <SwmToken path="cmd/security-agent/api/listener.go" pos="15:2:2" line-data="// newListener creates a listening connection">`newListener`</SwmToken> function creates a listening connection. It retrieves the IPC address from the configuration and attempts to create a TCP listener on the specified address and port. If successful, it returns the listener.
...
The <SwmToken path="cmd/security-agent/subcommands/start/command.go" pos="349:2:2" line-data="func setupInternalProfiling(config config.Component) error {">`setupInternalProfiling`</SwmToken> function configures and starts internal profiling for the security agent. It checks if internal profiling is enabled in the configuration and sets up the profiling settings such as the profiling URL, environment, service name, and various profiling parameters. If the <SwmToken path="cmd/security-agent/subcommands/start/command.go" pos="356:7:7" line-data="		// check if TRACE_AGENT_URL is set, in which case, forward the profiles to the trace agent">`TRACE_AGENT_URL`</SwmToken> environment variable is set, it forwards the profiles to the trace agent. Finally, it starts the profiling with the configured settings.
...
The <SwmToken path="cmd/security-agent/api/server.go" pos="53:2:2" line-data="// Start creates the router and starts the HTTP server">`Start`</SwmToken> function initializes and starts the HTTP server for the security agent. It creates the root HTTP router, sets up the IPC REST API server, and validates tokens for every request. It also generates a TLS certificate and configures the server to use TLS. The server is then started in a separate goroutine to handle incoming requests.
...
The <SwmToken path="cmd/system-probe/config/config.go" pos="46:2:2" line-data="// New creates a config object for system-probe. It assumes no configuration has been loaded as this point.">`New`</SwmToken> function creates a configuration object for the system probe by calling the <SwmToken path="cmd/system-probe/config/config.go" pos="48:3:3" line-data="	return newSysprobeConfig(configPath)">`newSysprobeConfig`</SwmToken> function with the provided configuration path.
...
The <SwmToken path="pkg/api/util/util.go" pos="43:2:2" line-data="// CreateAndSetAuthToken creates and sets the authorization token">`CreateAndSetAuthToken`</SwmToken> function creates and sets the authorization token. It ensures that the token is only created if it is not already set and uses the <SwmToken path="pkg/api/util/util.go" pos="55:10:10" line-data="	token, err = security.CreateOrFetchToken(config)">`CreateOrFetchToken`</SwmToken> function to generate or retrieve the token.
...
The <SwmToken path="cmd/security-agent/api/agent/agent.go" pos="45:2:2" line-data="// SetupHandlers adds the specific handlers for /agent endpoints">`SetupHandlers`</SwmToken> function adds specific handlers for the <SwmPath>[Dockerfiles/agent/](Dockerfiles/agent/)</SwmPath> endpoints. It sets up various HTTP handlers for endpoints like <SwmPath>[cmd/agent/subcommands/version/](cmd/agent/subcommands/version/)</SwmPath>, <SwmPath>[cmd/agent/subcommands/flare/](cmd/agent/subcommands/flare/)</SwmPath>, <SwmPath>[cmd/agent/subcommands/hostname/](cmd/agent/subcommands/hostname/)</SwmPath>, <SwmPath>[cmd/agent/subcommands/stop/](cmd/agent/subcommands/stop/)</SwmPath>, <SwmPath>[cmd/agent/subcommands/status/](cmd/agent/subcommands/status/)</SwmPath>, <SwmPath>[cmd/agent/subcommands/health/](cmd/agent/subcommands/health/)</SwmPath>, <SwmPath>[cmd/agent/subcommands/config/](cmd/agent/subcommands/config/)</SwmPath>, and <SwmToken path="cmd/security-agent/api/agent/agent.go" pos="59:6:9" line-data="	r.HandleFunc(&quot;/workload-list&quot;, func(w http.ResponseWriter, r *http.Request) {">`/workload-list`</SwmToken>.
...
The <SwmToken path="pkg/api/security/security.go" pos="129:2:2" line-data="// CreateOrFetchToken gets the authentication token from the auth token file &amp; creates one if it doesn&#39;t exist">`CreateOrFetchToken`</SwmToken> function retrieves the authentication token from the auth token file or creates one if it doesn't exist. It ensures that the configuration is set up before calling.
...
The <SwmToken path="cmd/system-probe/config/config.go" pos="51:2:2" line-data="func newSysprobeConfig(configPath string) (*types.Config, error) {">`newSysprobeConfig`</SwmToken> function creates a new configuration object for the system probe. It sets the configuration file name, adds the configuration paths, and loads the configuration. If there are any errors during loading, it handles them appropriately.
...
The <SwmToken path="pkg/process/events/listener_linux.go" pos="64:2:2" line-data="// NewSysProbeListener returns a new SysPobeListener">`NewSysProbeListener`</SwmToken> function creates a new <SwmToken path="pkg/process/events/listener_linux.go" pos="65:26:26" line-data="func NewSysProbeListener(conn *grpc.ClientConn, client api.EventMonitoringModuleClient, handler EventHandler) (*SysProbeListener, error) {">`SysProbeListener`</SwmToken> instance. It requires an <SwmToken path="pkg/process/events/listener_linux.go" pos="65:21:21" line-data="func NewSysProbeListener(conn *grpc.ClientConn, client api.EventMonitoringModuleClient, handler EventHandler) (*SysProbeListener, error) {">`EventHandler`</SwmToken> and initializes the <SwmToken path="pkg/process/events/listener_linux.go" pos="65:26:26" line-data="func NewSysProbeListener(conn *grpc.ClientConn, client api.EventMonitoringModuleClient, handler EventHandler) (*SysProbeListener, error) {">`SysProbeListener`</SwmToken> with the provided <SwmToken path="pkg/process/events/listener_linux.go" pos="65:7:7" line-data="func NewSysProbeListener(conn *grpc.ClientConn, client api.EventMonitoringModuleClient, handler EventHandler) (*SysProbeListener, error) {">`grpc`</SwmToken> connection, client, and handler.
